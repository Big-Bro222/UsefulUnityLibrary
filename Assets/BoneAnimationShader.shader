Shader "Custom/BoneAnimationShader"
{
	Properties
	{
		...
		[Toggle]
		_EnableRandomTime("Enable Random Time",int) = 0
	}
	SubShader
	{
		Tags
		...
		Pass
		{
			...
			float4 readInBoneTex(float total)
			{
				...
				float2 animUv = float2((newUv.x + 0.5) * _AnimTex_TexelSize.x,
				                       (newUv.y + 0.5 + UNITY_ACCESS_INSTANCED_PROP(Props, _Start)) * _AnimTex_TexelSize
				                       .y);

				float r = DecodeFloatRGBA(tex2Dlod(_AnimTex, float4(animUv, 0, 0)));
				newUv = uvConvert(total + 1);
				animUv = float2((newUv.x + 0.5) * _AnimTex_TexelSize.x,
				                (newUv.y + 0.5 + UNITY_ACCESS_INSTANCED_PROP(Props, _Start)) * _AnimTex_TexelSize.y);
			...
			v2f vert(appdata v, uint instanceID : SV_InstanceID)
			{
				...
				// 计算y轴坐标（时间）
				float y = _Time.y * _FrameRate + UNITY_ACCESS_INSTANCED_PROP(Props, _Offset) *
					UNITY_ACCESS_INSTANCED_PROP(Props, _FrameCount);

				#ifdef UNITY_INSTANCING_ENABLED
				if (_EnableRandomTime != 0)
				{
					float random;
				...
				#endif
				y = floor(
					y - floor(y / UNITY_ACCESS_INSTANCED_PROP(Props, _FrameCount)) * UNITY_ACCESS_INSTANCED_PROP(
						Props, _FrameCount));
